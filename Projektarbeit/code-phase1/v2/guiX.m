...
% Daten Ballform (Halbkugel, Sinus)
...
% Daten Berg und Tal - ähnlichen Daten
...
% Eingabe der x,y Position der gewünschten Stützstelle
...
%% plotten der Daten in Values als Stützstellen
for x=1:xmax
    for y=1:ymax
...
%% Lineare Interpolation zwischen 4 gegebenen Stützstellen
...
% handles    structure with handles and user data (see GUIDATA)
global x y values;
% Datenvorbereitung für Funktionsaufruf
u=[x y];
xlow= floor(u(1));
ylow= floor(u(2));
xhigh= ceil(u(1));
...
y=h(2);
h=h(3);
% Zeichnen der interpolierten Stützstelle in rot
gcf;
